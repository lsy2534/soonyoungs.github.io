...
여러 은닉 층(hidden Layer)을 쌓은 인공 신경망(ANN) 이런 이유로 딥러닝이 심층 신경망과 거의 동의가 되었다.
...
```
from tensorflow.keas iport optimizers
...
케라스는 많은 손실 함수를 기본으로 제공하며 자신만의 손실 함수를 정의할 수도 있습니다. 가장 많이 사용하는 세 개의 손실 함수는 편균 제곱 오차, 범주형 크로스 엔트로피와 이진 크로스 엔트로피 입다.
